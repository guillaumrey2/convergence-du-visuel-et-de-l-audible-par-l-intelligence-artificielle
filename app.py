...
def uploaded_file(filename):
    base_path = os.path.join(app.config['RECORDINGS_FOLDER'], filename.rsplit('.', 1)[0])
    audio_file_path = base_path + '.wav'
    done_flag_path = base_path + '.wav.done'

    if os.path.exists(done_flag_path):
        if os.path.exists(audio_file_path):
            return send_from_directory(app.config['RECORDINGS_FOLDER'], os.path.basename(audio_file_path))
        else:
